...
struct SupabaseAuthMiddleware: AsyncMiddleware {
    ...
    func respond(
        ...
        guard
            let base = Environment.get("SUPABASE_URL"),
            let key = Environment.get("SUPABASE_PUBLISHABLE_KEY")
        else {
        ...
        guard let url = URL(
            string: base.trimmingCharacters(
        ...
        guard response.status == .ok else {
            throw Abort(
                .unauthorized,
                reason: "ログインセッションが無効です"
            )
        }

        let user = try response.content.decode(
            SupabaseUser.self
        )
...
/// SupabaseAuthMiddlewareの後段で使うこと
/// (先にAuthenticatedUserがログイン済みである必要がある)。
